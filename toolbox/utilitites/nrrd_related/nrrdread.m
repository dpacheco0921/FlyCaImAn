...
%
% Args:
%   filename: reads the image volume and associated
%       metadata from the NRRD-format file specified by FILENAME.
%
% Returns:
%   X: matrix
%   meta: metadata from the NRRD-format file
%       (nrrd format specs: http://teem.sourceforge.net/nrrd/format.html)
%
% Notes:
% Copied and edited from http://www.mathworks.com/matlabcentral/fileexchange/34653-nrrd-format-file-reader?focused=6792784&tab=function
...
% fix bugs when some files do not have endian info
...
switch (meta.encoding)
    ...
    case {'gzip', 'gz'}
        ...
        fclose(fidTmp);
        [out_, out__] = system(['rm -f ' tmpFile]);
        [out_, out__] = system(['rm -f ' tmpBase]); 
        clear out_ out__
