function [Ain, Cin, bin, fin, center] = initialize_components_int(Y, K, tau, options, P)
...
if isfield(options, 'brainmask') && ~isempty(options.brainmask)
    Y = bsxfun(@times,Y,reshape(single(options.brainmask), d));
end
...
if options.noise_norm
     mY = mean(Y,ndims(Y));
     if isfield(options, 'brainmask') && ~isempty(options.brainmask)
        norm_image = mY + median(mY(options.brainmask > 0)) + 1e-4;
     else
