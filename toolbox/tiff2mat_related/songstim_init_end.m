...
if ~exist('minSize', 'var') || isempty(minSize); minSize = 10000; end

% raw input auditory stim
stimuli_cat_trace = [];
for j = rDat.sti
    stimuli_cat_trace = cat(1, stimuli_cat_trace, rDat.stimAll{j}(:, stimCha));
...
stimuli_onset_offset = [(find(diff(inputTrace_bi) == 1) + 1), find(diff(inputTrace_bi) == -1) + 1];
